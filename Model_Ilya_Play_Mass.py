...
class Type_a_2(mesa.Agent):
    ...
    def __init__(self, unique_id, model, pos, mass, viability_time, immediate_killing, agressiveness):
        super().__init__(unique_id, model)
    ...
    def eat(self):
        ...
        for nutrient in self.nutrition_list:
            ...
            if nutrient in soil.nutrients and soil.nutrients[nutrient] > 0:
                ...
                if self.immediate_killing == False:
        ...
        if self.mass >= self.split_mass:
            ...
            if self.has_eaten:
                ...
                if new_position != None:
                    ...
                    # creating and placing new bacteria
                    new_bacteria = Type_a_2(self.model.next_id(), self.model, new_position, self.mass / 2, viability_time, self.immediate_killing, agressiveness)
                    self.model.grid.place_agent(new_bacteria, new_position)
                    self.model.schedule.add(new_bacteria)
                else:
                    self.viability_index += 1
    ...
    def die(self):
             ...
             if (self.mass < self.min_mass) or (self.viability_index >= self.max_viability_time) or (self.random.random() < self.dying_chance) or (immediate_kill == True):
...
class Microbiome(mesa.Model):
    ...
    def __init__(self, num_type_a_1, num_type_a_2 ,is_torus, grid_height, grid_width, # Compulsory inputs for the simulation
                 avrg_mass_num_type_a = average_bacteria_mass, avrg_viability_time_type_a = viability_time, immediate_killing = False, agressiveness = agressiveness): # Variables that have a default value but can be changed
        ...
        for i in range(num_type_a_1):
            ...
            a = Type_a_1(self.next_id(), self, (x, y), avrg_mass_num_type_a, avrg_viability_time_type_a)
            self.schedule.add(a)
        ...
        for i in range(num_type_a_2):
            x = self.random.randrange(self.grid.width)
            y = self.random.randrange(self.grid.height)
            a = Type_a_2(self.next_id(), self, (x, y), avrg_mass_num_type_a, avrg_viability_time_type_a, immediate_killing, agressiveness)
            self.schedule.add(a)
            # Add the agent to a random grid cell
            self.grid.place_agent(a, (x, y))
